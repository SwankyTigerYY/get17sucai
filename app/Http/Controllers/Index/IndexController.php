...
class IndexController extends Controller
{
    ...
    {
        $url = "http://www.jq22.com/demo/treeSelect201905170004/ztree/img/";
        self::file_exists_S3($url, 'z_4.png', 'D:\phpStudy1\PHPTutorial\WWW\get17sucai-master\public\moban\\');
    ...
    public function index1(Request $request)
    {
                    ...
                    foreach ($match_css_result as $k => $v) {
                        ...
                        if (isset($css_src_result[1])) {
                            foreach ($css_src_result[1] as $kk => $vv) {
                                $cache_string = str_replace("'", "", $vv);
                                $cache_string = str_replace("\"", "", $cache_string);
                                if ($flag >= 1)$cache_string = str_replace("../", $filename_array[$flag-1] . '/', $cache_string);
                                if (isset(get_headers($url . $cache_string, true)[0]) && (strpos(get_headers($url . $cache_string, true)[0], '200') || strpos(get_headers($url . $cache_string, true)[0], '304'))) {
                                    self::file_exists_S3($url, $cache_string, $filepath);
                                }
                            }
        ...
        return view("index1.form")->with("");
    }

    /**
     * https://blog.csdn.net/muzi187/article/details/56486178?utm_source=blogxgwz5
    ...
    {
        file_put_contents('D:\phpStudy1\PHPTutorial\WWW\get17sucai-master\public\moban\log.txt', $url . '---' . $filename . '---' . $filepath . PHP_EOL, FILE_APPEND);
